...
class Peripheral(BluepyHelper):
    def __init__(self, deviceAddr=None, addrType=ADDR_TYPE_PUBLIC, iface=None,sec = "low", timeout=None):
        ...
        if isinstance(deviceAddr, ScanEntry):
            self._connect(deviceAddr.addr, deviceAddr.addrType, deviceAddr.iface, sec, timeout)
        elif deviceAddr is not None:
            self._connect(deviceAddr, addrType, iface, sec, timeout)
    ...
    def _connect(self, addr, addrType=ADDR_TYPE_PUBLIC, iface=None, sec = "low",timeout=None):
        print ("<_connect>")
        if len(addr.split(":")) != 6:
