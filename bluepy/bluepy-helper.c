...
static void cmd_remove(int argcp, char **argvp)
{
    ...
    if (mgmt_send(mgmt_master, MGMT_OP_REMOVE_DEVICE,
            mgmt_ind, sizeof(cp), &cp,
            remove_device_complete, NULL,
                NULL) == 0) {
        DBG("mgmt_send(MGMT_OP_REMOVE_DEVICE) failed for %s for hci%u", opt_dst, mgmt_ind);
        resp_mgmt(err_SEND_FAIL);
        return;
    }
}
