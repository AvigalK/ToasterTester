...
class Peripheral(BluepyHelper):
    def __init__(self, deviceAddr=None, addrType=ADDR_TYPE_PUBLIC, iface=None ,sec = "low"):
        ...
        if isinstance(deviceAddr, ScanEntry):
            self._connect(deviceAddr.addr, deviceAddr.addrType, deviceAddr.iface ,sec)
        elif deviceAddr is not None:
            self._connect(deviceAddr, addrType, iface, sec)
    ...
    def _connect(self, addr, addrType=ADDR_TYPE_PUBLIC, iface=None ,sec = "low"):
        print ("<_connect>")
        if len(addr.split(":")) != 6:
...
class Scanner(BluepyHelper):
    ...
    def process(self, timeout=10.0, deviceName = None):
        if self._helper is None:
